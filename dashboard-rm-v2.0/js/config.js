...
// Shim: DataTable global alias only exists in DataTables >= v1.13
if (typeof DataTable === 'undefined') {
  window.DataTable = function(selector, config) { return $(selector).DataTable(config); };
}

// ── Section cache helpers (used by home.js and sales.js) ──────────────────────
async function fetchCachedSection(section) {
  try {
    const r = await fetch('cache/read_cache.php?section=' + encodeURIComponent(section));
    if (!r.ok) return null;
...
async function refreshCacheSection(section) {
  try {
    const r = await fetch('cache/refresh_cache.php?section=' + encodeURIComponent(section));
    const j = await r.json();
    return j.success ? j : null;
  } catch { return null; }
}

// Auto-refresh cache every hour at the top of the hour while the page is open
(function scheduleCacheRefresh() {
  var MS = 60 * 60 * 1000;
  var msUntilNextHour = MS - (Date.now() % MS);
  setTimeout(function tick() {
    fetch('cache/refresh_cache.php?section=all').catch(function() {});
    setTimeout(tick, MS);
  }, msUntilNextHour);
}());